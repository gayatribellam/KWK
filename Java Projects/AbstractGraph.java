...
public abstract class AbstractGraph<V> implements Graph<V> {
	...
	public boolean isSubgraphCyclic(int v, boolean isVisited[], int parent) {
		isVisited[v] = true;
		for(Edge e: neighbors.get(v)) {
			if(!isVisited[e.v]) {
				if(isSubgraphCyclic(e.v,isVisited, v))
					return true;
			}
			else if(e.v != parent && parent !=-1)
				return true;
		}
	...
	public boolean isCyclic() {
		...
		for(int u=0; u < vertices.size(); u++) {
			if(!isVisited[u])
				if(isSubgraphCyclic(u,isVisited,-1))
					return true;
		}
		return false;
	}
	  
	public void getHamiltonianPaths() {
		for(int u=0; u < vertices.size();u++) {
			boolean[] isVisited = new boolean[vertices.size()];
			List<Integer> searchOrder = new ArrayList();
			searchOrder.add(u);
			isVisited[u] = true;
			getHamiltonianPaths(u,isVisited, searchOrder);
		}
		
	}
	
	public void getHamiltonianPaths(int u, boolean isVisited[], List<Integer> searchOrder ) {
		if(searchOrder.size()==vertices.size()) {
			for(int vertex: searchOrder)
		...
		for(Edge e:neighbors.get(u)) {
			if(!isVisited[e.v]) {
				...
				getHamiltonianPaths(e.v,isVisited, searchOrder);
				isVisited[e.v] = false;
